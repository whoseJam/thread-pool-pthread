...
public:
	...
	int queueSize;		pthread_mutex_t mutQS;		//pool/worker change queueSize , signal to worker
	int activeWorker;	pthread_mutex_t mutUpd;		//worker change activeWorker,signal to pool
	int queueIsEmpty;								//worker change queueIsEmpty,signal to pool
